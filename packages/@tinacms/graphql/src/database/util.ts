...
export const stringifyFile = (
  ...
  const { _relativePath, _keepTemplateKey, _id, _template, _collection, $_body, ...rest } = content as {
...
export const scanAllContent = async (
  ...
  bridge: Bridge,
  callback: (collection: Collection<true>, contentPaths: string[]) => Promise<void>
) => {
...
export const scanContentByPaths = async (
  ...
  callback: (collection: Collection<true> | undefined, documentPaths: string[]) => Promise<void>
) => {
  const { pathsByCollection, nonCollectionPaths, collections } = await partitionPathsByCollection(
    tinaSchema,
    documentPaths
  );
...
export const partitionPathsByCollection = async (tinaSchema: TinaSchema, documentPaths: string[]) => {
...
export const transformDocument = <T extends object>(
  ...
  const templateName =
    hasOwnProperty(contentObject, '_template') && typeof contentObject._template === 'string'
      ? contentObject._template
      : undefined;

  const { collection, template } = hasOwnProperty(contentObject, '__collection')
    ? {
        collection: tinaSchema.getCollection(contentObject['__collection'] as string),
        template: undefined,
...
export const getTemplateForFile = (
  ...
  if (templateInfo?.type === 'union') {
    if (hasOwnProperty(data, '_template')) {
      const template = templateInfo.templates.find((t) => lastItem(t.namespace) === data._template);
      if (!template) {
        throw new Error(
          `Unable to find template "${data._template}". Possible templates are: ${templateInfo.templates
...
export const loadAndParseWithAliases = async (
  ...
  const data = parseFile(dataString, path.extname(filepath), (yup) => yup.object({}), {
